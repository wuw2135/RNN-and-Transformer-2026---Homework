...
from .blocks.freqfusion import FreqFusion
...
class Detector(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.p5_to_p4 = FreqFusion(hr_channels=fpn_channels, lr_channels=fpn_channels)
        self.p4_to_p3 = FreqFusion(hr_channels=fpn_channels, lr_channels=fpn_channels)
        self.p3_to_p2 = FreqFusion(hr_channels=fpn_channels, lr_channels=fpn_channels)
    ...
    def forward(self, x1s, x2s):
        ...
        _, p4_hr, p4_lr_up = self.p5_to_p4(hr_feat=p4, lr_feat=fea_p5)
        fea_p4 = self.tb4(p4_hr + p4_lr_up)
        pred_p4 = self.p4_head(fea_p4)


        # fea_p3 = self.p4_to_p3(fea_p4, p3)
        # fea_p3 = self.tb3(fea_p3)
        _, p3_hr, p3_lr_up = self.p4_to_p3(hr_feat=p3, lr_feat=fea_p4)
        fea_p3 = self.tb3(p3_hr + p3_lr_up)        
        pred_p3 = self.p3_head(fea_p3)


        # fea_p2 = self.p3_to_p2(fea_p3, p2)
        # fea_p2 = self.tb2(fea_p2)
        _, p2_hr, p2_lr_up = self.p3_to_p2(hr_feat=p2, lr_feat=fea_p3)
        fea_p2 = self.tb2(p2_hr + p2_lr_up)
...
class SegModel(nn.Module):
    def __init__(self, backbone="mobilenetv2", fpn_channels=96, **kwargs):
        ...
        self.upsample1 = CARAFEPack(channels=12, scale_factor=2)
        self.upsample2 = CARAFEPack(channels=2, scale_factor=2)
    ...
    def forward(self, x1, x2):
        ...
        # print("fea1:", fea1[0].shape, fea1[1].shape, fea1[2].shape, fea1[3].shape)
        # print("fea2:", fea2[0].shape, fea2[1].shape, fea2[2].shape, fea2[3].shape)

        # print("dino p2:", d_p2.shape, "dino p3:", d_p3.shape, "dino p4:", d_p4.shape, "dino p5:", d_p5.shape)



        t1_p2 = self.fuse1_p2(fea1[0], d_p2)
        # print("t1_p2:", t1_p2.shape)
        t1_p3 = self.fuse1_p3(fea1[1], d_p3)
        # print("t1_p3:", t1_p3.shape)
        t1_p4 = self.fuse1_p4(fea1[2], d_p4)
        # print("t1_p4:", t1_p4.shape)
        t1_p5 = self.fuse1_p5(fea1[3], d_p5)
        # print("t1_p5:", t1_p5.shape)

        t2_p2 = self.fuse2_p2(fea2[0], d_p2)
        # print("t2_p2:", t2_p2.shape)
        t2_p3 = self.fuse2_p3(fea2[1], d_p3)
        # print("t2_p3:", t2_p3.shape)
        t2_p4 = self.fuse2_p4(fea2[2], d_p4)
        # print("t2_p4:", t2_p4.shape)
        t2_p5 = self.fuse2_p5(fea2[3], d_p5)
        # print("t2_p5:", t2_p5.shape)
...
if __name__ == "__main__":
    ...
    # 解析命令列參數（會帶預設值；你也可在執行時覆蓋）
    args = Options().parse()

    # 你要測 128x128x12，就確保參數一致
    args.input_size = 128
    args.fpn_channels = 96
    args.backbone = "lwganet"   # 你訓練指令裡用的是 lwganet
    args.segmodel = "dino"
    args.create_model = "base"
    ...
    # 建兩個假的輸入：x1 是 12 通道、x2 是 2 通道（你的 SegModel 就是這樣設計的）
    x1 = torch.randn(1, 12, args.input_size, args.input_size).cuda()
    x2 = torch.randn(1,  2, args.input_size, args.input_size).cuda()

    with torch.no_grad():
        preds, t1_feats, t2_feats = model(x1, x2)

    # 看一下輸出形狀
    print(type(preds))
    if hasattr(preds, "shape"):
        print("preds.shape:", preds.shape)
    elif isinstance(preds, (list, tuple)) and len(preds)>0 and hasattr(preds[0], "shape"):
    ...
    summary(
    model,
    input_data=(x1, x2),  # 你的模型 forward 需要兩個 input
    depth=3,              # 顯示深度，越大越詳細
    col_width=18,
