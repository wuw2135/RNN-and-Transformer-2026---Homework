...
class DRFD(nn.Module):
    def __init__(self, dim, norm_layer, act_layer):
        ...
        self.outdim = dim * 2
        self.conv = nn.Conv2d(
            dim, dim * 2, kernel_size=3, stride=1, padding=1, groups=dim
        )
        self.conv_c = nn.Conv2d(
            dim * 2, dim * 2, kernel_size=3, stride=2, padding=1, groups=dim * 2
        )
    ...
    def forward(self, x):  # x = [B, C, H, W]
        ...
        x = torch.cat(
            [conv, max], dim=1
        )  # x = [B, 2C+2C, H/2, W/2]  -->  [B, 4C, H/2, W/2]
...
class LA(nn.Module):
    def __init__(self, dim, norm_layer, act_layer):
        ...
        self.conv = nn.Sequential(
            nn.Conv2d(dim, dim, 3, 1, 1, bias=False), norm_layer(dim), act_layer()
        )
...
class LWGA_Block(nn.Module):
    def __init__(
        self, dim, stage, att_kernel, mlp_ratio, drop_path, act_layer, norm_layer
    ):
        super().__init__()
        self.stage = stage
        ...
        self.MRA = MRA(
            self.dim_split, att_kernel, norm_layer
        )  # MRA is medium-range attention
        if stage == 2:
            self.GA3 = D_GA(
                self.dim_split, norm_layer
            )  # GA3 is global attention (stage of 3)
        elif stage == 3:
            self.GA4 = GA(self.dim_split)  # GA4 is global attention (stage of 4)
            self.norm = norm_layer(self.dim_split)
        else:
            self.GA12 = GA12(
                self.dim_split, act_layer
            )  # GA12 is global attention (stages of 1 and 2)
            self.norm = norm_layer(self.dim_split)
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        x1, x2, x3, x4 = torch.split(
            x, [self.dim_split, self.dim_split, self.dim_split, self.dim_split], dim=1
        )
        x1 = x1 + self.PA(x1)
        x2 = self.LA(x2)
        x3 = self.MRA(x3)
...
class BasicStage(nn.Module):
    def __init__(
        self, dim, stage, depth, att_kernel, mlp_ratio, drop_path, norm_layer, act_layer
    ):
...
class Stem(nn.Module):
    ...
    def __init__(self, in_chans, stem_dim, norm_layer=nn.BatchNorm2d):
        super().__init__()
        ...
        **kwargs,
        ...
        for i_emb, i_layer in enumerate(self.out_indices):
            if i_emb == 0 and os.environ.get("FORK_LAST3", None):
                ...
                layer = nn.BatchNorm2d(int(stem_dim * 2**i_emb))
...
def LWGANet_L0_1242_e32_k11_GELU(embed_dim=32, pretrained=False, **kwargs):
    model = LWGANet(
        in_chans=3,
        stem_dim=embed_dim,
        depths=(1, 2, 4, 2),
    ...
    if pretrained:
        checkpoint = torch.load(
            "/ssd4/chingheng/CD-work/OurCD/model/backbone_weights/lwganet_l0_e299.pth",
...
def LWGANet_L1_1242_e64_k11_GeLU(embed_dim=64, pretrained=False, **kwargs):
    model = LWGANet(
        in_chans=3,
        stem_dim=embed_dim,
        depths=(1, 2, 4, 2),
    ...
    if pretrained:
        checkpoint = torch.load(
            "/ssd4/chingheng/CD-work/OurCD/model/backbone_weights/lwganet_l1_e299.pth",
            map_location=torch.device("cuda:0"),
        )
        _state_dict = checkpoint["model"]
        state_dict = _state_dict
...
def LWGANet_L2_1442_e96_k11_ReLU(embed_dim=96, pretrained=False, **kwargs):
    model = LWGANet(
        in_chans=3,
        stem_dim=embed_dim,
        depths=(1, 4, 4, 2),
    ...
    if pretrained:
        checkpoint = torch.load(
            "/ssd4/chingheng/CD-work/OurCD/model/backbone_weights/lwganet_l2_e296.pth",
            map_location=torch.device("cuda:0"),
        )
        _state_dict = checkpoint["model"]
        state_dict = _state_dict
