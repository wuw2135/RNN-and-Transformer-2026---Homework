...
class DifChA(nn.Module):
    def __init__(self, dim, num_heads, depth, bias):
        super(DifChA, self).__init__()
        assert dim % num_heads == 0
        self.num_heads = num_heads
        self.head_dim = dim // num_heads
    ...
    def forward(self, x):
        ...
        # logits
        logits1 = (q1 @ k1.transpose(-2, -1)) * self.dot_scale  # b head c c
        logits2 = (q2 @ k2.transpose(-2, -1)) * self.dot_scale  # b head c c
        ...
        out = (attn1 - lambda_full * attn2) @ v  # b head c n

        # group norm (num_head groups)
        out = self.group_norm(out) * (1 - self.lambda_init)  # b head c n
...
class TransformerBlock(nn.Module):
    def __init__(
        ...
        # self.channel_attn = DifChA(dim, num_channel_heads, depth, bias)
